...
## الهدف
مرجع موحّد لبروتوكولات العمل اليومي مع Claude Code على مشروع MoradBot.
...
- **القسم 1 (نص CLAUDE.md كاملاً)** — استُبدل بإحالة للملف الأصلي؛ تكرار المحتوى يخلق تضارباً عند التحديث
- **القسم 2 (per-app CLAUDE.md)** — تم التخلي عن النهج كلياً؛ الملفات الثلاثة حُذفت
- **القسم 4 (مخطط 16 Hook)** — استُبدل بالـ 5 Hooks الفعلية من `.claude/settings.json`
- **القسم 5 (agents متقادمة)** — أُزيلت الـ Agents المؤرشفة (dx-optimizer، sql-pro، frontend-developer، إلخ)
- **تقنيات خاطئة في القسم 1** — Vanilla TypeScript → Preact + Vite، React + Vite → Next.js 15 App Router
