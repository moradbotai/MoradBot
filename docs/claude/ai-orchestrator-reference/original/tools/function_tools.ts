...
const ModifyCartInput = z.object({
	customerId: z.string().describe("The ID of the customer."),
	itemsToAdd: z
		.array(z.object({ product_id: z.string(), quantity: z.number() }).passthrough())
		.describe("A list of objects, each with 'product_id' and 'quantity'."),
	itemsToRemove: z
		.array(z.object({ product_id: z.string() }).passthrough())
		.describe("A list of items to remove (often just product_id)."),
});
