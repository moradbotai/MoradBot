...
function setupEventDelegation(viewPhotoCallback) {
	...
	container.addEventListener("click", (e) => {
		...
		if (action === "camera") {
			...
			// Similarly for help
			const itemSchema = Object.values(CHECKLIST_SCHEMA)
				.flat()
				.find((i) => i.id === target.dataset.itemId);
...
async function handleItemUpdate(itemId, value) {
	...
	if (ui.hideChecked) {
		...
		const itemSchema = Object.values(CHECKLIST_SCHEMA)
			.flat()
			.find((i) => i.label === itemId || i.id === itemId);
		const rows = document.querySelectorAll(
			`.item-row[data-id="${itemSchema ? itemSchema.id : itemId}"]`
		);
