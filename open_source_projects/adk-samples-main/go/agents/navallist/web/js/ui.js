...
export const ui = {
		...
		if (type === "state") {
			...
			if (data) {
				Object.values(data).forEach((client) => {
					if (client) {
						const userId =
							client.user || client.userId || client.userID || client.UserID || client.User || "";
						if (userId) {
							let name = userId;
	...
	updateItemVisibilityByTripType() {
		Object.values(CHECKLIST_SCHEMA)
			.flat()
			.forEach((item) => {
	...
	showHelp(item) {
		...
		const explanation = this.getItemExplanation(item);
		document.getElementById("help-explanation").innerText =
	...
	updateTabVisibility() {
		...
		const myTab = Array.from(document.querySelectorAll(".tab")).find(
			(t) => t.innerText === "My Items"
		);
		if (myGroup && myTab) {
		...
		const currentTab = Array.from(document.querySelectorAll(".tab")).find(
			(t) => t.innerText === this.currentCategory
		);
	...
	applyItemState(row, item, state, input) {
		...
		let assignName =
			state[`${item.label}_assigned_to_name`] || state[`${item.id}_assigned_to_name`];
	...
	updateMyItemsTab(state, currentUserId, currentDisplayName) {
		...
		const cleanUserId = currentUserId.startsWith("guest_")
			? currentUserId.substring(6)
			: currentUserId;
		...
		Object.values(CHECKLIST_SCHEMA)
			...
			.forEach((item) => {
				let assignName =
					state[`${item.label}_assigned_to_name`] || state[`${item.id}_assigned_to_name`];
				let assignId =
					state[`${item.label}_assigned_to_user_id`] || state[`${item.id}_assigned_to_user_id`];

				// Robust match: check full ID, clean ID, or Name (from state, presence, or passed display name)
				let match =
					(assignId && (assignId === currentUserId || assignId === cleanUserId)) ||
					(assignName &&
	...
	renderReportContent(items, trip) {
		...
		Object.keys(CHECKLIST_SCHEMA).forEach((category) => {
			...
			CHECKLIST_SCHEMA[category].forEach((schemaItem) => {
				...
				const displayName =
					dbItem && dbItem.completed_by_user_name
						? dbItem.completed_by_user_name
						: dbItem
							? dbItem.completed_by_name
							: null;
				if (displayName) {
