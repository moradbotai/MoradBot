...
import { Video, Mic, Loader2, X, Monitor, Camera } from "lucide-react";
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
			<div className="bg-gray-800 rounded-lg shadow-2xl p-8 max-w-sm w-full relative">
				...
				<h2 className="text-2xl font-semibold mb-6 text-center">Choose your video source</h2>
...
export default function Home() {
	const { connectionState, latestTextMessage, eventLog, connect, disconnect } = useLiveConnection();
	...
	return (
		<main className="flex flex-col h-screen w-full">
			...
			{showSourceModal && (
				<SourceModal onSelect={handleStartStream} onClose={() => setShowSourceModal(false)} />
			)}
			...
			<footer className="w-full p-4 flex justify-center items-center gap-4 bg-gray-900 border-t border-gray-700">
				{!isStreaming && !isConnecting ? (
					<>
						...
						<span className="text-gray-400">Click the icon to start recording</span>
					</>
				) : (
					<>
						<button
