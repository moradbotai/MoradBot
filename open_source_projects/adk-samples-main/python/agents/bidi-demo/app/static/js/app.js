...
function addConsoleEntry(type, content, data = null, emoji = null, author = null, isAudio = false) {
	...
	typeLabel.textContent =
		type === "outgoing" ? "↑ Upstream" : type === "incoming" ? "↓ Downstream" : "⚠ Error";
				...
				if (currentInputTranscriptionId == null) {
					...
					if (currentOutputTranscriptionId == null && currentMessageId == null) {
						if (isFinished) {
							...
							const existingText =
								currentInputTranscriptionElement.querySelector(".bubble-text").textContent;
			...
			if (transcriptionText) {
				...
				if (currentOutputTranscriptionId == null) {
					...
					if (isFinished) {
						...
						// Partial transcription - append to existing text
						const existingText =
							currentOutputTranscriptionElement.querySelector(".bubble-text").textContent;
						// Remove typing indicator if present
						const cleanText = existingText.replace(/\.\.\.$/, "");
						updateMessageBubble(
