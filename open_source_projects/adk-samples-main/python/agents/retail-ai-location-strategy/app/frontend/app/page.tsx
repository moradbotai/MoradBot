	...
	useCoAgentStateRender<AgentState>({
		...
		render: ({ state }) => {
			...
			if (
				...
				state.pipeline_stage === "intake" ||
				!state.stages_completed?.length
			) {
			...
			const currentLabel =
				stageLabels[state.pipeline_stage] || `Processing ${state.pipeline_stage}...`;
			...
			return (
				<div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
					<div className="flex items-center justify-between">
						...
						<span className="text-xs text-gray-500">{completedCount}/7 complete</span>
					</div>
				</div>
			);
		},
			...
			<main className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
				<div className="max-w-5xl mx-auto p-8">
					...
					<header className="mb-8">
						<h1 className="text-4xl font-bold text-gray-900 mb-2">Retail AI Location Strategy</h1>
						<p className="text-gray-600">Powered by Google ADK + Gemini 3 | Multi-Agent Pipeline</p>
					</header>
					...
					{state?.strategic_report && (
						<div className="space-y-6">
							...
							{state.strategic_report.alternative_locations?.length > 0 && (
								<AlternativeLocations locations={state.strategic_report.alternative_locations} />
							)}
					...
					{!state?.target_location && (
						<div className="bg-white rounded-xl shadow-sm border p-12 text-center">
							...
							<h2 className="text-3xl font-bold text-gray-900 mb-4">Find Your Perfect Location</h2>
							<p className="text-gray-600 max-w-lg mx-auto mb-8 text-lg">
								Tell me where you want to open your business in the chat, and I'll run a
								comprehensive analysis using live market data, competitor mapping, and AI-powered
