...
export const CollapsibleStep = memo(function CollapsibleStep({
	...
	return (
		<div className={`rounded-lg border transition-all duration-300 ${styles.container}`}>
			{/* Step Header */}
			<div
				className={`flex items-center gap-3 p-4 ${canToggle ? "cursor-pointer select-none" : ""}`}
			...
			>
				...
				<span className={`font-medium flex-1 ${styles.textColor}`}>{step.label}</span>
			...
			<div
				className={`overflow-hidden transition-all duration-300 ease-in-out ${
					isExpanded && status !== "pending" ? "max-h-[32rem] opacity-100" : "max-h-0 opacity-0"
				}`}
			>
