...
import { summarizeCompetitorAnalysis } from "@/lib/summaryHelpers";
...
export function StepOutputContent({ stepId, state }: StepOutputContentProps) {
	switch (stepId) {
		case "intake":
			return (
				<p className="text-gray-600 text-sm">
					Analyzing <span className="font-medium text-gray-900">{state.business_type}</span>{" "}
					locations in <span className="font-medium text-gray-900">{state.target_location}</span>
		...
		case "market_research":
			return (
				<ScrollableMarkdown content={state.market_research_findings || ""} maxHeight="12rem" />
			);

		case "competitor_mapping":
			// Show summary stats at top, then full content if available
			const summary = summarizeCompetitorAnalysis(
			...
			return (
				<div className="space-y-2">
					...
					{state.competitor_analysis && (
						<ScrollableMarkdown content={state.competitor_analysis} maxHeight="12rem" />
					)}
				</div>
			);
		...
		case "gap_analysis":
			return (
				<TabbedGapAnalysis content={state.gap_analysis || ""} code={state.gap_analysis_code} />
			);
		...
		case "strategy_synthesis":
			...
			return (
				<div className="space-y-2">
					...
					<p className="text-sm text-blue-700">"{rec.opportunity_type}" opportunity</p>
				</div>
			);
						...
						<div className="flex gap-2">
							<button
								onClick={() => {
									...
									if (win) {
										win.document.write(
											`<img src="${state.infographic_base64}" style="max-width:100%;"/>`
										);
									}
								}}
								className="px-3 py-1 text-xs bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors"
